...
void begin(const char* a) {
	glBegin(primitive_from_string(a));
}
...
A buffer on the graphics.
	.bind(string what, string type, int stride, int offset, int size = 0, int texnum = 0)
		"vertex"
		"color"
		"texcoord"
...
vbo::vbo(PyObject* o, const char* t) {
	...
	glBindBuffer(GL_ARRAY_BUFFER, id);
	glBufferData(GL_ARRAY_BUFFER, PyBytes_Size(o), const_cast<const char*>(PyBytes_AsString(o)), buffer_usage_from_string(t));
...
void vbo::bind(const char* what, const char* type, int stride, int offset, int size, int num) {
	GLint last;
	glGetIntegerv(GL_ARRAY_BUFFER_BINDING, &last);
	...
	if(!strcmp(what, "color"))
		glColorPointer(size, common_type_from_string(type), stride, (const GLvoid*) offset);
	else if(!strcmp(what, "vertex"))
		glVertexPointer(size, common_type_from_string(type), stride, (const GLvoid*) offset);
	else if(!strcmp(what, "texcoord")) {
		GLint act;
		glGetIntegerv(GL_ACTIVE_TEXTURE, &act);
		glActiveTexture(GL_TEXTURE0 + num);

		glTexCoordPointer(size, common_type_from_string(type), stride, (const GLvoid*) offset);

		// Reset to the original texture
		if(act != GL_TEXTURE0 + num) glActiveTexture(act);
	}
	else
		err("vbo", "bind", "invalid bind");
...
vbo_enable('color')
* */
void vbo_enable(const char* a, bool b) {
	GLenum e;
	...
	if(!strcmp(a, "vertex"))
		...
		e = GL_COLOR_ARRAY;
	else if(!strcmp(a, "texcoord"))
		e = GL_TEXTURE_COORD_ARRAY;
	else {
		err("vbo_enable", "invalid option");
		return;
	}
