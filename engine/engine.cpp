...
sound::sound(const char* a) {
	buffer = alureCreateBufferFromFile(a);
...
void sound::pan(float a) {
	...
	//alSource3f(source, AL_DIRECTION, a * M_PI * 10000.0, 0.0, 0.0);
}
...
void display::draw(float x, float y, float sx, float sy, float r, float xo, float yo) {
	...
	if(r) {
		...
		glTexCoord2i(0, 0); glVertex2i(x + (-xo * rc - -yo * rs) * sx, y + (-yo * rc + -xo * rs) * sy);
		glTexCoord2i(1, 0); glVertex2i(x + ((current_image->w - xo) * rc - -yo * rs) * sx, y + (-yo * rc + (current_image->w - xo) * rs) * sy);
		glTexCoord2i(1, 1); glVertex2i(x + ((current_image->w - xo) * rc - (current_image->h - yo) * rs) * sx, y + ((current_image->h - yo) * rc + (current_image->w - xo) * rs) * sy);
		glTexCoord2i(0, 1); glVertex2i(x + (-xo * rc - (current_image->h - yo) * rs) * sx, y + ((current_image->h - yo) * rc + -xo * rs) * sy);
	}
	else {
		glTexCoord2i(0, 0); glVertex2i(x - xo * sx, y - yo * sy);
		glTexCoord2i(1, 0); glVertex2i(x + (current_image->w - xo) * sx, y - yo * sy);
		glTexCoord2i(1, 1); glVertex2i(x + (current_image->w - xo) * sx, y + (current_image->h - yo) * sy);
		glTexCoord2i(0, 1); glVertex2i(x - xo * sx, y + (current_image->h - yo) * sy);
	}
}
...
void display::image_set(const char* a) {
	if (!strcmp(a, "linear")) {
